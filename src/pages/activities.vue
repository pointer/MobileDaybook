...
export default {
  ...
  mounted () {
    ...
    // debugger;
    moment.locale('fr')
    // document.addEventListener("deviceready", this.displayData, false);
  ...
  methods: {
    ...
    // debugger;
      navigator.notification.confirm(
        'Voulez-vous verifier si les vacations\nsont déjà dans votre Calendrier?', // message
    ...
    createEvents () {
      ...
      for (let todo in todos) {
        if (todos.hasOwnProperty(todo)) {
          ...
          let site = todos[todo].assign_site.replace(/<\/?[^>]+(>|$)/g, "");
          let task = todos[todo].assign_tasks.replace(/<\/?[^>]+(>|$)/g, "");
          let obs = todos[todo].assign_observations; //.replace(/<\/?[^>]+(>|$)/g, "");
          let eventLocation =
            site +
            ' ' +
